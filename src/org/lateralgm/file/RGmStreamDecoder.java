...
public class RGmStreamDecoder extends StreamDecoder {
    protected int originalPos = -1;
    protected InputStream originalStream;
    private int[] table = null;

    public RGmStreamDecoder(InputStream in) {
        super(in);
    }

    public RGmStreamDecoder(String path) throws FileNotFoundException {
        super(path);
    }

    public RGmStreamDecoder(File f) throws FileNotFoundException {
        super(f);
    }
...
/*
    ...
    /**
     * Safely finishes this stream if it's an inflater, otherwise this call does
     * nothing. This places the file reader after the end of the compressed data
    ...
    public BufferedImage readBGRAImage(int w, int h) throws IOException
		...
		if (s != data.length)
			throw new IOException(
					"GmStreamDecoder.IMAGE_SIZE_MISMATCH"); //$NON-NLS-1$
     ...
     * you may wish to delay setting the seed until that byte is retrieved, as
     * implementing such functionality into these lower-level routines would add
     * overhead
     */
    ...
    protected static int[] makeDecodeTable(int seed) {
        int[] encTable = makeEncodeTable(seed);
